...
import com.where2move.app.config.KafkaProperties;
...
class Where2MoveKafkaResourceIT {
    ...
    void setup() {
        ...
        Where2MoveKafkaResource kafkaResource = new Where2MoveKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/where-2-move-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/where-2-move-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
